           ...
           <div class="row"></br>
                <div class="col-lg-12">
                    ...
                    <!-- /.panel -->
									...
									<a href='edit_roster?eve=$event1&rid=$roster1&sta=$state1&gen=$gender' class='btn btn-warning'><i class='fa fa-edit fa-fw'></i> Edit Roster</a>
									</center>
									</div>
									 <hr>";
                     ...
                     <button type="button" class="close" data-dismiss="modal">&times;</button>
                       <h4 class="modal-title text-center">Reject Roster</h4>
                                  </div>
                            <div class="modal-body text-center">
							<p><i class="fa fa-exclamation-circle fa-3x"></i></p>
                          <p><b>Are you sure you want to reject this roster?</b></p>
					...
					if(isset($_POST['reject']))
					{
						...
						$sql1 = "update abfi_roster_create set status='2' WHERE roster_id='$roster1'";
						$delete_player = $form->extract($sql1);
						
						if($delete_player)
						{
						  $sql2 = "update abfi_roster_create_off set status='2' WHERE roster_id='$roster1'";
						  $delete_offc = $form->extract($sql2);
						  if($delete_offc)
						  {
						  echo "<script>alert('Roster rejected...');</script>";	  
				...
				<div class="modal fade" id="myModal1" role="dialog">
                     ...
                     <button type="button" class="close" data-dismiss="modal">&times;</button>
                       <h4 class="modal-title text-center">Approve Roster</h4>
                            ...
                            <div class="modal-body text-center">
							<p><i class="fa fa-exclamation-circle fa-3x"></i></p>
                          <p><b>Are you sure you want to approve this roster?</b></p>
						  <form method="post" action="">
						  <input type="hidden" value="<?php echo $roster;?>" name="del_play"/>
					...
					{
						$roster1 = $_POST["del_play"];
						$sql1 = "update abfi_roster_create set status='3' WHERE roster_id='$roster1'";
						$delete_player = $form->extract($sql1);
						
						if($delete_player)
						{
						  $sql2 = "update abfi_roster_create_off set status='3' WHERE roster_id='$roster1'";
						  $delete_offc = $form->extract($sql2);
						  if($delete_offc)
						  {
						  echo "<script>alert('Roster approved...');</script>";	  
						  header( "refresh:0;url=roster_data_tbl" );
						  }
						}
					}
