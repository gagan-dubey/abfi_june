...
<body>
						...
						<div class="col-md-6 col-sm-offset-3">
		                                   <div class="form-group">
											 ...
											 <option disabled="" selected="">--Select Event--</option>
														...
														<?php
                                                           $all_events = $form->get_events_all_new();
		                                   ...
		                                   <div class="form-group">
										    ...
										    <div class="col-md-6 col-sm-offset-3">
		                                   <div class="form-group">
                                             <select name="egender" class="form-control" id="egender" required>
											 <option disabled="" selected="">--Select Type--</option>
 ...
 <script type="text/javascript">
    $(function () {
        $("#egender").change(function () {
            var selectedText = $('#etype').find("option:selected").text();
            var selectedValue = $('#etype').val();
...
if(isset($_POST["ply"]))
{
	...
	$gender = base64_encode($_POST["egender"]);
	$event1 = $_POST["event"];
	$etype1 = $_POST["etype"];
	$egender1 = $_POST["egender"];
	
	if(empty($etype) || empty($event))
	{
		echo "<script> alert('Missing Fields'); </script>";
	    header("refresh:0;url=gen_cert");
	}
		...
		else
		{
			if($etype1 == 'P')
			{
			header("refresh:0;url=gen_cer/index?eve=$event&gen=$gender");
			}
...
if(isset($_POST["off"]))
{
	...
	if(empty($etype) || empty($event))
	{
		...
		else
		{
			if($etype1 == 'O')
			{
			header("refresh:0;url=gen_cer_offc/index?eve=$event");
			}
