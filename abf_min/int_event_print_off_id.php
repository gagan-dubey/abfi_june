...
<title>Home | ABFI | INDIA | ID-CARDS | OFFICIALS</title>
...
<a href='int_event_print_id?<?php echo"eve=$event1&rid=$roster1";?>'><button><i class='fa fa-arrow-circle-left fa-fw'></i> Back</button></a>
...
<?php 
$sql1 = "SELECT a.off_id AS reg_id, a.off_name AS name, a.off_dob AS dob, a.off_img as img, a.off_bloodg as blood, a.off_position as position, a.off_country as country, a.status as status, c.event_dates as dates FROM  intn_eve_off_roster AS a
		 INNER JOIN abfi_tour_create as c ON a.event_id = c.sec_event_id where a.roster_id = '$roster' and a.event_id = '$event'";
$result1 = $form->extract($sql1);
if (mysqli_num_rows($result1) > 0) {
	while($row = mysqli_fetch_assoc($result1)) {
						        $a =$row["reg_id"];
								$b =strtoupper($row["name"]);
								$c =strtoupper($form->get_country_name($row["country"]));
						        $d =$row["dob"]; 
								$e =$row["img"];
								$f =strtoupper($form->get_offc_name($row["position"]));
								$g =$row["blood"];
								$i =$row["dates"];
								$j = substr($i,0,10);
echo"
...
<center><img class='idlogo' src='int_eve_imgs/no-image.png' style='height:140px; width:120px; margin-top:0px; border:4px solid #022a5e;'></center>
<br>
...
<div id='tbl1'><div id='tbl2'><label class='headingid7'>Country</label></div><div id='tbl3'>$c</div></div>
<div id='tbl1'><div id='tbl2'><label class='headingid7'>Validity</label></div><div id='tbl3'>$j</div></div>
<div id='tbl3'></div></div></div><div id='bt'><img src='../images/id_bk212.jpg'></div>
</div>";
}}
else 
{
