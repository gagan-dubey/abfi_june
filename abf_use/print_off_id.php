...
<title>Home | ABFI |  <?php echo $form->get_state_name($state);?>  | ID-CARDS | OFFICIALS</title>
...
<a href='print_id?<?php echo"eve=$event1&rid=$roster1";?>'><button><i class='fa fa-arrow-circle-left fa-fw'></i> Back</button></a>
...
<?php 
$sql1 = "SELECT a.off_reg_id AS reg_id, b.full_name AS name, b.father_name AS father, b.dob AS dob, b.user_img as image, b.blood_group as blood, b.position as position, b.emergency_contct as econtact, a.status as status, c.event_dates as dates FROM abfi_roster_create_off AS a
		 INNER JOIN abfi_ply_offc AS b ON a.off_reg_id = b.offc_reg_id INNER JOIN abfi_tour_create as c ON a.event_id = c.sec_event_id where a.roster_id = '$roster' and a.event_id = '$event'";
...
if (mysqli_num_rows($result1) > 0) {
	while($row = mysqli_fetch_assoc($result1)) {
								...
								$i =$row["dates"];
								$j1 = substr($i,13);
								$j2 = date("d-m-Y", strtotime($j1));
								$j = date('d-m-Y', strtotime($j2. ' + 4 days'));
echo"
...
<div id='tbl1'><div id='tbl2'><label class='headingid7'>Valid Up-to</label></div><div id='tbl3'>$j</div></div>
