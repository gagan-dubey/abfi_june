...
$status = 0;
$sql = "INSERT INTO abfi_roster_create_off(off_reg_id,roster_id,event_id,state,status) VALUES ('$selected','$roster_id','$event','$state','$status')";
$roster1 = $form->extract($sql);
}
if ($roster1) 
			{
				echo"<script>alert('Officials added successfully');</script>";
				header("refresh:0;url=roster_view?eve=$eve&rid=$enc_roster_id");
			} 
else 
			{
