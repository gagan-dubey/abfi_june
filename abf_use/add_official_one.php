                ...
                <div class="card wizard-card" data-color="blue" id="wizardProfile">
                    <form action="" method="POST" id="defaultForm" enctype="multipart/form-data">
                    	...
                    	<div class="wizard-header">
                        	<h3>
                        	   <small>Official Registration Stage - 1</small>
                        	</h3>
                    	</div>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane" id="address">
                               ...
                               <div class="form-group">
                                          ...
                                          <select name="designation" class="form-control" id="position" required>
										   <option disabled="" selected="">--Select--</option>
											<?php
											$state =  $_SESSION["state"];
											$check = new abfi_use();
											$offc_check = $check->check_official($state);
											$result = mysqli_num_rows($offc_check);
											if ($result == 0)
											{
											...
											elseif ($result > 0)
											{
												while($row = mysqli_fetch_assoc($offc_check))
												{
...
if(isset($_POST["finish"]))
{
	...
	$offc_id = $code . "OFFC" . rand(10,1000);
	$sec_offc_id = $code . "OFFC" . rand(10,10000);
	...
	$position = 201;
	$designation = $form->mysql_prep($_POST["designation"]);
	...
	if($image_input == 4)
	{
		...
		if($empty_check == 3)
		{
			...
			if($insert_data)
			{
			$sql ="update abfi_login set abfi_status='2' where id='$userid'";
			$status=$form->extract($sql);
			if($status)
			{
			$new_token = $form->update_token($userid);
			if($new_token)
			{
			$get_new_token = $form->get_token($userid);
			echo "<script> alert('Data Entered Successfully'); </script>";
			 header("refresh:0;url=add_official_two?tnk=$get_new_token");
			}
			
			else
				{
				   echo"<script>alert('Error Contact Your Admin....');</script>";
				}
			...
			else
		{
			echo "<script> alert('Error in entering data..!!!'); </script>";
			 header("refresh:0;url=add_official_one");
		}
